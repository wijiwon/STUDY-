...
// 사용할수있다.
// 클래스를 사용하는 이유
// extends키워드를 사용해서 클래스를 상속 시킬수있다.

// 부모클래스의 기능을 상속 받아서 사용 할수있다.
...
class Cat extends Animal{
    ...
    // run이라는 함수가 재정의 되면 재정의된 함수를 사용할수있다.(함수 오버라이딩)
    run(speed){
        this.speed = speed;
        console.log(`${this.name}은 ${this.speed}로 달리는중 나는 자식의 오버라이딩 함수임`);
    }
    speak(){
        console.log("야옹야옹~");
    }
    stop(){
        // 재정의 했음 
        // 부모의 stop을 실행 -> 재정의 해서 사용할수는 있는데
        // 상속받은 부모의 클래스 키워드로 부모의 함수를 실행 시킬수있다.
...
// 일반적인 함수는 new 키워드를 함깨 사용하면 빈객체가 만들어 지는데 this라는 키워드에
// 객체를 참조 시킨다.
